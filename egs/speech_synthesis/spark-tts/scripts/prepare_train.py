...
def process(meta: Dict[str, Any], tokenizer: AutoTokenizer, code_dir: Path, gt_num: int) -> Dict[str, Any]:
    ...
    global_tokens_tensor, semantic_tokens_tensor = tokens[:gt_num], tokens[gt_num:]

    # Create the strings for global and semantic tokens
    global_tokens = "".join([f"<|bicodec_global_{i}|>" for i in global_tokens_tensor])
    semantic_tokens = "".join(
        [f"<|bicodec_semantic_{i}|>" for i in semantic_tokens_tensor]
    )

    # ---- START FINAL CHECK AND ASSERTION ----
    # For the specific file we are debugging, let's make sure semantic_tokens is not empty.
    if "m3ed_Angry_0000000014" in meta["index"]:
        num_semantic_ints = len(semantic_tokens_tensor)
        print(f"\n--- FINAL CHECK FOR {meta['index']} ---")
        print(f"Found {num_semantic_ints} semantic token integers.")
        print(f"Resulting semantic_tokens string length is {len(semantic_tokens)}.")
        # If this assertion fails, the script will crash, proving the error.
        assert len(semantic_tokens) > 0, "CRITICAL ERROR: semantic_tokens string is empty when it should not be!"
        print("Assertion passed. The semantic_tokens string is correctly generated.")
        print("--- END FINAL CHECK ---\n")
    # ---- END FINAL CHECK AND ASSERTION ----
...
def process_data_multithread(metadata, tokenizer, code_dir, gt_num, max_workers=128):
    """
    Process metadata in a single thread for debugging purposes.
    """
    print("\n--- RUNNING IN SINGLE-THREAD DEBUG MODE ---\n")
    tts_data = []
    control_tts_data = []

    for meta in tqdm(metadata, desc="Processing data (single-thread)"):
        outputs = process(meta, tokenizer, code_dir, gt_num)
        index = outputs['index']
